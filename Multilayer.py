...
class StartPageGUI(tk.Frame):
	...
	def __init__(self, parent, controller, runObject):
		...
		self.logo = PhotoImage(file=os.path.join('/multilayer/MULTILAYER-master/logo', 'multilayer2.png'))
	...
	def analysisGO(self):
		...
		if 'All - Communities' in self.titleGO:
			...
			for i in self.dic_Com_ListGene.keys():
				self.dfBank = pd.read_csv(os.path.join('/multilayer/MULTILAYER-master/GO_DB', f'{self.comboGOBank.get()}.tsv'), sep='\t', index_col=[0])
...
class mainGUI(object):
	...
	def loadGOBank(self):
		...
		"""
		list_GO_Bank_temp = []
		for file in os.listdir('/multilayer/MULTILAYER-master/GO_DB'):
